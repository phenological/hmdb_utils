...
Implement the multiplet searcher
```{r}
library(dplyr)
library(stringdist)
library(stringr)
...
approximate_lookup <- function(df, column, search_string, scorer1 = "lv", scorer2 = "lcs", limit = 10) {
  ...
  matches <- matches %>% filter(score > 0.3)
  ...
  if (nrow(matches) > 0) {
    ...
    for (i in 1:nrow(matches)) {
      ...
      matchings_names <- df[match_index, 'synonyms']
...
result
```
Implement name lookup function
```{r}
result$similarity
```
